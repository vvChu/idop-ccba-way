...
- Focused diff in `tools/scripts/sp-diff.ps1`:
  - Parameters:
    - `-Focus all|issues|changed` (default `all`)
    - `-OnlyLists <CSV of list names>`
    - `-SinceGit <ref>` (optional)
  - State caching: `.serena/state/sp-diff-last.json` stores per-list result and JSON hash to enable fast re-runs.
- Targeted + fast apply in `tools/scripts/apply-sp-lists.ps1`:
  - Parameters:
    - `-OnlyLists <CSV of list names>`
...
```powershell
# From repo root
powershell -ExecutionPolicy Bypass -File .\tools\scripts\sp-diff.ps1
```

### Fast re-run: only prior failures
```powershell
powershell -ExecutionPolicy Bypass -File .\tools\scripts\sp-diff.ps1 -Focus issues
```

### Fast re-run: only lists whose JSON changed
```powershell
powershell -ExecutionPolicy Bypass -File .\tools\scripts\sp-diff.ps1 -Focus changed
```

### Limit to a small set
```powershell
powershell -ExecutionPolicy Bypass -File .\tools\scripts\sp-diff.ps1 -OnlyLists CDEDocuments,PotentialProjects,Opportunities
```

### Apply quickly to a small set (skip validation)
```powershell
powershell -ExecutionPolicy Bypass -File .\tools\scripts\apply-sp-lists.ps1 -OnlyLists CDEDocuments,PotentialProjects,Opportunities -Fast
```

## Notes and guardrails
- State file `.serena/state/sp-diff-last.json` is updated after each diff run; `-Focus issues` and `-Focus changed` use that cache.
...
- If the cache file becomes corrupted, delete `.serena/state/sp-diff-last.json` and run a full scan to rebuild.
